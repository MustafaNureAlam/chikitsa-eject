...
export default function Saturday({navigation, route}) {

    const isMounted = useRef(true);

    const [day_id, setDayId] = useState('6');
    ...
    async function getDoctorSlots() {
        ...
        await fetch( app_config.baseUrl + "doctor/my_slots/6", requestOptions)
        .then(response => response.text())
        .then(result => {
            let doctor_slot_data = JSON.parse(result);
            console.log('=====doctor_slot_data=====' )
            console.log(doctor_slot_data )
            console.log('=====doctor_slot_data=====' )
