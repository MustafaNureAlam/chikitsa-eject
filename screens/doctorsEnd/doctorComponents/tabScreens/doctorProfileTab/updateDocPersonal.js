...
export default function UpdateDocPersonalData({navigation, route}) {
    const isMounted = useRef(true);
    // console.log('routeParams===========', route)
    const [patientName, setPatientName] = useState(route.params?.data?.name);
    const [doc_licence, setLicence] = useState(route.params?.doctor_info?.license_number);
    const [doc_type, setDocType] = useState(route.params?.doctor_info?.doc_type);
    const [doc_type_list, setDocTypeList] = useState([]);
    ...
    async function getDocType(){
        ...
        await fetch(app_config.baseUrl + "doctor/get_type", requestOptions)
        .then(response => response.text())
        .then(result => {
            let jsonResult = JSON.parse(result);
            setDocTypeList(jsonResult?.data)
            // console.log('doctor_profile',doctor_profile)

        })
        .catch(error => console.log('[doctor/get_type] - error', error));
                    ...
                    <View style={{marginHorizontal:20,marginVertical:20}}>
                        <View>
                            ...
                            <View style={{
                                ...
                                <RNPickerSelect
                                    onValueChange={(type) => setDocType(type)}
                                    items={doc_type_list}
                                    value={doc_type}
