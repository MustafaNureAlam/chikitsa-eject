...
export default function ConcernDocDetailsScreen({navigation, route}) {
    ...
    const [advance_payment, setAdvancePayment] = useState(0);
    const [doctorAvailableTimeFrom, setDoctorAvailableTimeFrom] = useState({});
    const [is_slot, setIsSlot] = useState(false);
    const [emptyRbPageMessage, setEmptyRbPageMessage] = useState("");

    const doctor_appoint_props = {
        doctor_user_id : doc_details?.id,
        appointment_date: appointment_date,
        slot_id: selectedId,
        appointment_time : doctorAvailableTimeFrom?.available_time_start_from?.start_from,
        minimum_advance : advance_payment,
        type : "doctor"
    }

    async function set_selected_slot(slot_id, slot_item) {
        setSelectedId(slot_item.id);
        setAppointTime(slot_item.time_from);
        setAdvancePayment(slot_item.minimum_advance)
        slot_item['appointment_date'] = appointment_date
        let availableTimeStartFrom = await patientEnd.getAvailableTimeStartFrom(slot_item)
        
        setDoctorAvailableTimeFrom(availableTimeStartFrom)
        console.log("availableTimeStartFrom######")
        console.log(slot_item)
        console.log(availableTimeStartFrom)
        console.log("availableTimeStartFrom######")
    }
    ...
    async function getDoctorSchedule() {
        if(doctor_appoint_props.doctor_user_id){
            ...
            setIsSlot(true);
            if(!(doctor_schedule_data?.data[0]?.slots?.length > 0)){
                setEmptyRbPageMessage("Sorry ! No slot available right now")
            }
            setLoading(false);
        }
    }
    ...
    useEffect(() => {
        if(isMounted) {
            setLoading(true);
            setDoctorAvailableTimeFrom({})
            setEmptyRbPageMessage("Loading...")
    ...
    function pay_proceed() {
        ...
        // });
        if(doctorAvailableTimeFrom?.available_time_start_from?.is_available){
            navigation.navigate('PaymentScreenStack',{screen: 'PaymentScreen', params: doctor_appoint_props});
        }else{
            alert(doctorAvailableTimeFrom?.available_time_start_from?.start_from);
        }
    }
                                            ...
                                            >
                                                ...
                                                <View style={{
                                                    ...
                                                    <Text 
                                                        ...
                                                        Minimum advance : {item?.minimum_advance}
                                                    </Text>
                                                    
                                                    {doctorAvailableTimeFrom?.available_time_start_from && (
                                                        <Text 
                                                            style={{
                                                                fontSize:16,
                                                                fontWeight:'700',
                                                                color: doctorAvailableTimeFrom?.available_time_start_from?.is_available ? '#fff' : '#ffcf52',
                                                                // textAlign:'center'
                                                            }}>
                                                            Available Time:  {doctorAvailableTimeFrom?.available_time_start_from?.start_from}
                                                        </Text>
                                                    )}
