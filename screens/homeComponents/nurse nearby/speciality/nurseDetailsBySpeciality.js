...
export default function NurseDetailsBySpecialityScreen({navigation, route}) {
    ...
    async function getNurseSchedule(signal) {
        if(doctor_appoint_props.doctor_user_id){
            let nurse_schedule_data = await patientEnd.nurseSchedule(doctor_appoint_props, signal)
            setLoading(true);
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            setLoading(true);
            getNurseSchedule(schedule_cancel);
            setLoading(false);
            return () => {
